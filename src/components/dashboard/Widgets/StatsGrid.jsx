  ...
  <BentoCard>
    <div className="flex flex-col h-full gap-4"> 
      ...
      <div className="mt-auto">
        ...
        <p className={`text-xs mt-1 font-medium ${subValue ? 'text-gray-500' : 'text-transparent'}`}>
            {subValue || "-"}
        </p>
      </div>
    </div>
...
const StatsGrid = ({ stats, expensesCount }) => {
  ...
  const onlineTotal = stats.modeStats?.find(s => s._id === 'Online')?.total || 0;
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
      ...
      <StatCard 
        title="Top Category" 
        value={stats.categoryStats.sort((a,b) => b.total - a.total)[0]?._id || "-"} 
